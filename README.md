...
Проект задокументирован с помощью OpenAPI 3. Просмотр структуры эндпоинтов, DTO-моделей и отправка тестовых запросов:
* Интерактивный UI: http://localhost:8080/swagger-ui.html
* Спецификация в формате JSON: http://localhost:8080/v3/api-docs
* Спецификация API без запуска приложения: https://xbobrov.github.io/accounting/ ()
...
В репозиторий включен дамп тестовой базы данных ([`db_backup.sql`](https://github.com/xBobrov/accounting/blob/master/src/main/resources/db_backup.sql)), содержащий демонстрационный набор данных.
...
* Разработка механизма авторизации внешних и внутрених пользователей информационной системы.
* Расширение функционала API. 
* Доработка моделируемых бизнес процессов: механизм перерасчетов потребления услуг, учет услуг водоснабжения по нежилым и производственным помещениям, обмен информацией с системой ГИС ЖКХ.  
* Разработка почтового сервиса: рассылка квитанций на оплату, информационных писем.
* Дальнейшее покрытие кода тестами и доработка документации.
